...
ASAM OpenCRG 1.2.1 is backward compatible to OpenCRG 1.0.0. 
To assure full backward and foreward compatibility do not use the features for geographical positions, which are introduced in <<Mapping OpenCRG data to geographical positions>>. A OpenCRG file created with the mpro field is only valid within version 1.2.0 or higher.
