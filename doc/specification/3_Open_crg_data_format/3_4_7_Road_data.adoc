...
* The _road data_ may contain NaN values, but there must be at least one record per lateral profile (i.e. per given u-position) that contains an entry with a number.
* NaN values must not occur between non-NaN records within a lateral profile. This means NaN values have always to be entered from the left and/or the right border to the actual road data entries.
